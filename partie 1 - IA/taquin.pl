...
%    heuristique1(U, H).  % au debut on utilise l'heuristique 1 
   ...
   % même piece.
	...
	malplace(P,U,F) :-
    	coordonnees([L,C], U,P), coordonnees([L1,C2],F,P), P\=vide , [L,C]\=[L1,C2].



   
    heuristique1(U, H) :-
    	 final_state(F), findall(P, malplace(P,U,F), L), length(L, H).
   ...
   % entre sa position courante et sa positon dans l'etat final
	...
	distance(P,U, F, R) :-
    	coordonnees([L,C], U, P), coordonnees([L1,C1], F, P), P\=vide, L2 is abs(L-L1), C2 is abs(C-C1), R is (L2+C2).


   
    heuristique2(U, H) :-
    	final_state(F), findall(R, distance(_, U, F, R), L), sumlist(L,H).
